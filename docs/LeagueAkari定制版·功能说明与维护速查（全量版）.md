...
- 自动同步范围：文字、符文基石、核心装备和可识别的召唤师/出门装图片来自**同一份工作簿快照**。表格及 Data Dragon 资源缓存为 10 分钟；选人页面可见且处于本局时每分钟检查，手动刷新立即重读；对局徽标重新悬停也检查时效。这是按需刷新，不是实时订阅；显示本机成功读取时间，不冒充作者修改时间。
...
- 故障回退：XLSX 下载/解析失败则尝试 gid=1026317672 的 CSV，保留新文字，**不把旧图片拼到新 CSV 上**；图片源故障缓存一分钟后允许重试。两种导出都失败时旧表仅供阅读。原历史人工补充入口仅在图片不可用时保留，默认关闭且明确标注；不能用历史配置填补在线图片冲突。对局徽标改为显示在线图片状态和来源行，不再默认显示历史技能/出门装。
- 界面：OP.GG 攻略卡和对局徽标共用图片状态组件，显示「图片已同步 / 图片需核对 / 图片暂不可用 / 旧图片已停用」、来源页/行、资源版本；未能采用的字段显示原因，可查看经解码缩小的原图标。读取公开导出不执行宏、公式或外链图，不写入游戏内存，不模拟键鼠；下载、XML、图片解压和尺寸均有限额。
...
| Bz Google Sheets XLSX / CSV 文字回退                                                                         | 劫攻略                                         | 唯一表头/英雄行、图片内容与位置、当前资源库；同一工作簿快照                           | 源异常标记旧表并停止覆盖；新英文原文回落              |
| OP.GG Mayhem 英雄页面                                                                                        | KIWI 强化及四类装备推荐                        | 英雄/模式/补丁校验，专属强化资源；不把未公开装备统计当成可靠数据                      | 缺失分区留空，缺名称显示编号                          |
| Data Dragon `versions.json` / `item.json` / `summoner.json`                                                  | 装备 id、图标                                  | `maps["11"]`、purchasable                                                             | 保留文本攻略                                          |
