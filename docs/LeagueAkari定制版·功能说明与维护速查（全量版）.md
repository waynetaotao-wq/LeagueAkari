...
**资料范围**：本轮内置 **27 个己方英雄、155 个有方向且限定分路的条目、32 个公开来源页面**，包含瑞兹对菲兹、菲兹对瑞兹两个独立方向。尚未覆盖全部英雄和组合。只提炼已读过的专精/长期攻略作者公开文字，不填充通用模板、不反转另一方心得、不跨分路套用。版本较旧的来源和排除内容逐项记入资料索引；页面版本标题不保证正文逐段更新，作者当前段位与攻略当前版本实战有效性未认证。
...
**实现与验证**：逻辑、数据、测试和预览集中在 `player-info-card/matchup-tips/`，集成点 `PlayerInfoCardHeader.vue`；未增加依赖、shard、窗口或写接口。本地整仓 node/web Typecheck、生产构建通过，Vitest **141 文件 / 977 项通过**，其中新增 14 项覆盖完整阵容、阶段、方向/分路隔离、Zed 排除、无资料、游戏编号及资料来源契约。构建仍有既有 ChampionIcon `@reference` 和大分块提示。Browser 使用真实 Vue/Naive UI 与玩家卡片标题组件，检查了 230px 卡片的长名字省略、深浅主题、双向中文、无链接弹窗，以及选人/资料缺失/错误对位/断开/退局等显示条件。合成阵容和预览只能验证程序与界面，未完成国服真实加载、实际换线、Windows 安装包实机及攻略逐条实战验收；不能称为全英雄覆盖或“精准度超高”。推送后另核对实际 SHA 的 Actions 和 Release，旧提交构建成功不替代本轮交付。
