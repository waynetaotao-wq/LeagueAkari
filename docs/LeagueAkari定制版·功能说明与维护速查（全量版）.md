...
**第一眼信息**：左侧列出至多三个优先禁用英雄，显示对应对手及常玩场次；右侧在本人熟练英雄中给出至多三个选人参考，显示历史场次、对位数据覆盖率及需要注意的对位。默认使用本人近期该位置至少玩过两场的英雄；展开“英雄池与依据”后可手动选择至多十个本局熟练英雄，或恢复自动。位置优先使用本局客户端指派，缺失时仅在历史有明确多数时推测，也可手动指定位置和敌方玩家。

**动态规则**：跟随已完成的禁用/选择事件更新；鼠标预选不视为已锁定。已锁英雄以当前成员字段为准，避免交换英雄后仍使用原操作记录。禁用避开我方预选及熟练候选；选人排除已禁、已被他人选走、禁用和客户端不可选英雄。本人锁定后保留当前英雄的对位参考。新会话清除本局手动选择，旧请求取消或迟到时不能写回新结果。
...
**禁选排序**：禁用按对手常玩占比、样本量及平滑后的历史表现排序，至少需要两场该英雄记录。选人按本人候选对整个目标英雄池的排位对位表现排序，对小样本收缩、薄弱对位和缺失覆盖作扣减，辅以本人历史熟练场次。单组对位少于 50 场不作为可靠统计；有效覆盖不足 60% 显示“暂不排名”。覆盖率表示具备足够数据的分析权重，不是优势比例，也不是本局胜率；该启发式排序未经真实五排胜负效果校准，不能称为稳定必胜或完美克制。

**对位统计与更新**：经既有 `champion-data` 通道读取 OP.GG 最新补丁、韩国翡翠+排位对位 JSON，明确显示补丁及“非五排专属胜率”。保持独立查询参数，不修改 OP.GG 窗口偏好；校验来源、英雄、位置、补丁及有效胜负样本，不反转胜率，不跨源补数。并发最多两项、30 秒请求期限、五分钟缓存/刷新，最多保留六十项缓存；异常可重试，禁用建议仍可使用已加载英雄池。展开表格可核对本人每个候选对各敌方英雄的真实样本数；原文/网页缺少的对位留空。
