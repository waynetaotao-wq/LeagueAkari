...
**生命周期**（`matchup-lifecycle.ts`、`OpggCounterIntel.vue`）：选人期只有敌方 assignedPosition 唯一、英雄已选出才直接采用；hover/pick intent 不作为已确定英雄。位置缺失时按当前补丁、地区和段位的全队分路先验推测，倾向值低于 0.6 则等待或手动指定。倾向百分比不是实测识别准确率，换路、摇摆位不能保证自动识别。加载后以真实阵容按位置校验（每 3 秒重试，最多 40 次）→ 全程锁定（含重连）→ 结算/大厅解锁。跨局用 champSelect session id + gameId 隔离；KeepAlive 切断、非活跃相位 reset。分路下拉始终可用，手动修正优先于选人指派；跨局清除手动选择。
...
- 源：Google Sheets `BZ_SHEET_ID=1FInDZ2JhIyto2y-FnCcgCVlAYcjRaF7egcpsV41Spic` gid=1026317672 的 CSV，五个必需表头严格校验（Champion/Rune/Difficulty/Core Build/Summary），仅劫触发。2026-09-12 在线表为 **63 行**，新增盖伦、内瑟斯；数量为本次观测值，不硬编码限制新行。
- 自动同步范围：文字、符文基石与核心装备按在线 CSV 更新；表格及 Data Dragon 装备映射缓存均为 10 分钟。选人页面可见时每分钟检查时效，手动刷新立即重读；对局徽标重新悬停时也会检查表格缓存。显示的是本机成功读取时间，不冒充作者修改时间。
- 装备安全：只认 Data Dragon `maps[11]` 的峡谷装备，竞技场变体排除。箭头顺序和斜杠备选全部映射成功才形成推荐；任何未知项都保留原文，不跳过中间装备拼出残缺构筑。旧表或旧装备库回退必须标注，旧表仅供阅读，不进入推荐及自动应用。
- 召唤师/出门装来自表格图片，**CSV 无法自动读取或校验图片变更**。历史人工补充仍可选择使用，但默认关闭，明确标注非实时同步。历史 61 行的默认多兰剑+闪现点燃及已校对例外保留；新增盖伦、内瑟斯没有人工图片校对，不生成默认配置。徽标中的历史补充也标注来源边界。
...
## 4. 数据源与外部契约（下表含历史契约；2026-09-12 复测范围见 §3.13）
...
| Bz Google Sheets CSV                                                                                         | 劫攻略                                         | 五表头、当前 63 行、内容指纹、读取时间；图片不在 CSV 中                               | 源异常标记旧表并停止覆盖；新英文原文回落              |
...
- Bz 图片列不能随 CSV 自动同步；Mayhem 来源仍有部分缺名资源；任意非好友及跨区观战受客户端许可限制，详见 §3.2、§3.3、§3.12。
