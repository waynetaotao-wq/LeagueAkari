...
**2026-09-14 好友申请流程测试**：在工具集 → 好友工具新增按时长运行的测试，间隔支持小数秒及 0 秒，提供开始、暂停、继续、停止，以及“接受后删除并继续”开关。按用户最新要求取消 30 秒下限；展示实际发送间隔和失败 HTTP 状态，429 即停止。入口、关系保护和真实客户端待验范围见 §3.16。
...
| 验证能力     | 2026-09-14 最新整仓 147 文件 / 1,049 项 Vitest、node/web Typecheck、生产构建通过；好友测试的 Electron 界面与 IPC 流程已用模拟接口检查，真实好友收发及限流未实测，见 §3.16。官方同步的验证记录见 §3.15 |
...
**已验证**：新增 28 项 Vitest 覆盖自定义时长、暂停冻结/继续、接受后保留/删除、接受与撤回竞争、原有关系保护、请求消失、不确定删除不重发、断线/换号、取消、429 停止、0/0.05/0.5/1 秒无隐藏下限，以及 PUUID/好友 ID 的 URL 编码和实际 Axios 自动重试关闭。最新整仓 147 文件 / 1,049 项测试、node/web Typecheck、生产构建通过。隔离资料目录下真实 Windows Electron 43.2.0 的 36 个主进程 shard 初始化成功；实际操作中文输入框和开始/暂停/继续/停止按钮，核对 0、0.5 秒参数经过真实 IPC 到达主进程、暂停冻结、离开页面暂停，以及浅色/深色和较窄窗口显示。模拟好友接口验证接受后删除及 429 的界面结果；Storybook 夹具仅为展示状态，不会发申请。

**尚未验证**：本轮未连接真实 League Client，未实际向任何账号发送、撤回或删除好友，未测出服务器限流。用户已明确选择使用自己的测试小号自行实测；模拟结果不能替代这些验收。推送后仍须检查同一提交的 Actions 和 Release，再提供其 Windows 包。
...
2026-09-14 实际运行 `src/**/*.test.ts`，共 **145 个文件 / 1,021 项通过**；数量随代码变动，以当次实际执行结果为准。定制测试包括 `matchup-build*.test.ts`、`bz-guide.test.ts`、`bz-summary-zh.test.ts`、`BzSummaryBadge.test.ts`、`matchup-lifecycle*.test.ts`、`refocus-logic.test.ts`、`akari-*.test.ts`、`midlane-research-*.test.ts`、`report-controller*.test.ts` 等；本轮还补充网络适配器、旧设置迁移及本地化网页口径测试。
