...
### 3.17 五排选人助手（2026-09-18；09-19 审查改进）

**入口与范围**：仅“积分对战 五排”（Ranked Flex，queue 440）的选人阶段，在主窗口及独立对局窗口的玩家卡片上方展示。只使用本次客户端选人会话中直接可见的对手身份；隐藏姓名、空身份和混淆身份不参与。断线、其他模式、观战、历史草稿、进入加载或对局后撤下。不会自动禁用或选择英雄，不改变 Bz 攻略、对线心得及原有选人预测。
...
**英雄池口径**：复用对局页已有 LCU/SGP 战绩，不新增身份查询。每人只计已加载、90 天内最近至多 50 场有效 queue 440、召唤师峡谷经典对局，去重并排除重开、短局、无效时间、身份不唯一及结果缺失记录；并非生涯全量样本。SGP 使用明确的 teamPosition；LCU 缺少可靠位置时标注全位置参考，不靠英雄名称猜位置。敌方本局位置不明时综合历史位置，保留多个可能对位和缺失样本的不确定性；可手动纠正换线。至多分析十二个权重最高的对手英雄，遗漏部分仍计入未覆盖比例。权重不是对方实际选人概率。
