# LeagueAkari 定制版 · 功能说明与维护速查（全量版 2026-09-04）
...
> 本次仅迁移手册、更新协作流程与项目入口；正文未另标日期的功能、测试计数、样本结论和待办仍是 2026-09-04 的历史记录，不能视为本次重新验证。
...
**2026-09-04 历史交付记录**：当时基线为 `631e3c91a980b1ffbecf3895989ccceb6ecd4cc5`，中单/评分修复包已上传，新「对局复盘台」42 文件包当时待上传。§8 保留原交付记录；本次未逐包审计这些历史状态，不据此重传旧包。功能详见 §3.11。
...
| 定制规模     | 原文约 70 文件为历史估计；本轮新增 38 文件、修改 4 个既有文件，未增加依赖/shard/窗口                                                                                                       |
| 验证能力     | 本轮使用真实仓库依赖完成 155 项 Vitest、整仓 node/web Typecheck 与生产构建；Windows 实机/客户端及截图未验证。上传后仍看 Actions                                                            |
...
### 3.11 对局复盘台（2026-09-04 新交付）

四项功能在一个原版 Naive UI 弹窗内联动：**关键片段、对位档案、时间线地图、领先兑现**。本次不改变既有评分公式，也不把统计相关性当成比赛输赢的原因。

**入口与使用**：

1. 玩家战绩页侧栏 →「个人复盘台」→「打开复盘台」。展开合格的峡谷战绩卡片，也可点「关键片段与地图复盘」直达该局。
2. 赛后小结会读取该局的关键片段，按钮可在主窗口打开完整复盘。赛后小窗只预览，不写入长期档案。
3. 历史分析需手动点「读取历史摘要」，最多扫描最近 500 场摘要；按英雄、位置、对位、队列、版本筛选后，分析最近 20 / 40 / 60 场时间线。默认优先实际存在的劫中单、单双排及所选队列最近版本，选项全部来自已读到的数据。
4. 「单局复盘」展示关键片段、地图、事件、经济曲线和 10/15/20 分钟快照；「对位档案」展示对线统计、近期/此前趋势和可编辑笔记；「领先兑现」按领先条件回看赢局或输局。

**四项功能的口径**：

| 功能       | 实际输出                                                                                                                               | 精度和限制                                                                                                                     |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 关键片段   | 领先后经济差收缩、带终结赏金的死亡、3 分钟内重复死亡、死亡后 90 秒内敌方获得建筑/史诗野怪；每局最多 5 条，附时间与事实依据             | 规则筛出的复盘线索；死亡与后续目标仅为时间关联，不称为因果或「送掉比赛」                                                       |
...
- 帧间隔超过 75 秒时，经济图断线，不跨缺口识别经济收缩。个人/团队经济差在 2–5 分钟内从领先收缩 ≥750/2000 金才产生该类线索。
...
- 成功缓存按区服/玩家/对局隔离，最多 120 局、15 分钟；不缓存时间线或事件不完整的结果。失败可见，可重试或继续未完成部分；刷新单局同步更新地图、当前批次统计和档案。
...
- 纯逻辑：`types.ts`、`analysis.ts`、`statistics.ts`、`test-fixtures.ts`。
...
- 单局联动：`ReviewMatchView.vue`、`ReviewTimeline.vue`、`ReviewMoments.vue`、`ReviewGoldChart.vue`、`review-view-utils.ts`、`review-view-text.ts`。
- 仅修改 4 个原有文件：主窗口 `App.vue`、战绩侧栏 `PlayerTabSidebarContent.vue`、战绩卡 `MatchHistoryCard.vue`、赛后 `PostGameView.vue`。卡片虚拟化保留；没有新增依赖、shard、窗口或构建配置。

**本次验证**：新增目录 10 个测试文件 75 项 + 中单/评分/赛后既有 9 个测试文件 80 项，共 **19 文件 155 项全部通过**；整仓 node 与 web Typecheck（含 noUnusedLocals/noUnusedParameters）通过；42 文件经 TypeScript AST/Vue 编译器解析、模板标签和 const/let/function/import 审计，零语法错误；原仓 Prettier、git diff --check 通过；`electron-vite build` 主进程、预加载及全部渲染窗口构建通过。

**未完成的环境验证**：本轮未连接用户的国服客户端读取真实对局；预览浏览器无法访问本地服务，因此未做真实截图/Windows 窗口视觉验收，也未本地打包 Windows 安装包。已有实际 Vue 时间线组件交互测试，但不能代替真机视觉检查。构建存在既有 ChampionIcon 的 `@reference` 警告及大分块提示，构建退出码为 0。

**用户安装后验收**：

1. 上传本包后等 Typecheck/Build Releases；下载安装 `league-akari-win-x64.7z`，核对页脚提交号。上传前的基线号 `631e3c9` 不是新包安装后的目标号。
2. 打开一局合格的劫中单战绩，确认时间、英雄、胜负一致；点击关键片段检查地图、事件、经济图定位联动。
...
- 新复盘台待用户上传、GitHub Actions 及 Windows/国服实际对局验收；目前本地类型、测试与构建通过，未获得真实界面截图。
